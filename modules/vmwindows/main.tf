resource "azurerm_network_interface" "vm_nic5456" {
  name                = "5456vm-nic-windows"
...
resource "azurerm_virtual_machine" "windows_vm" {
  ...
  location              = var.location
  resource_group_name   = var.resource_group_name
  network_interface_ids = [azurerm_network_interface.vm_nic5456.id]
  vm_size               = var.vm_size
...
os_profile_windows_config {
  ...
  availability_set_id = azurerm_availability_set.windows_availaibility_set.id
...
resource "azurerm_availability_set" "windows_availaibility_set" {
